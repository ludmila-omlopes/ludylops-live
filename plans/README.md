...
| 019  | Enforce module authorization and dependency-safe transitions | P1 | L | 009; coordinate 018 | [#185](https://github.com/ludmila-omlopes/ludylops-live/issues/185) | IMPLEMENTED (PR #198; 726 tests; awaiting merge; dependency policy, server guards, entry-point matrix and PostgreSQL concurrency checks) |
...
- 010–013 (added 2026-07-07 at `f245ee1`) come from the scalability review and are mutually independent; recommended order 010 → 011 → 012 → 013. Mapping to the review's five walls: 010 = invocation cost (wall 3, part of 1); 011 = missing cache (walls 1+2, the biggest capacity multiplier); 012 = per-request cost in `repository.ts` (wall 4 — surgical fixes only, decomposition stays deferred); 013 = push-vs-poll architecture ahead of white-label multiplication (wall 5, design doc only). 013 should conclude before onboarding the first external creator; 010/011 should land before any audience-growth push.
