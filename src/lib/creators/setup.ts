...
export function buildCreatorSetup(facts: SetupFacts): CreatorSetup {
  ...
  return { displayName: creator.displayName, active, currencyLabel, steps: [
    ...
    { id: "chat", title: "Ganhos por mensagem", state: !economy || !integration ? "blocked" : chat.enabled ? "verify" : "pending", detail: !economy || !integration ? blocked : chat.enabled ? `Regra salva: ${chat.amount} ${currencyLabel}, com intervalo de ${chat.cooldownSeconds}s por pessoa. Envie uma mensagem real e confira o crédito; salvar a regra não instala a action.` : "Defina a quantidade e o intervalo entre ganhos, ative a regra e configure a action de chat no Streamer.bot.", href: section("economia"), link: "Configurar ganhos no chat" },
