...
export function buildCreatorSetup(facts: SetupFacts): CreatorSetup {
  ...
  const path = `/c/${encodeURIComponent(creator.slug)}`;
  ...
  return { displayName: creator.displayName, active, currencyLabel, steps: [
    { id: "profile", title: "Nome e moeda", state: active ? "configured" : "blocked", detail: active ? `${creator.displayName} · ${currencyLabel}. Confira o nome e as cores que sua comunidade vai usar.` : blocked, href: `#perfil-${creator.id}`, link: "Revisar nome e cores" },
    { id: "economy", title: "Moeda disponível", state: economy ? "configured" : "blocked", detail: economy ? `Saldo e movimentações em ${currencyLabel} estão disponíveis. Cada streamer mantém sua própria moeda.` : blocked, ...(economy ? { href: `${path}/moeda`, link: "Conferir moeda e vínculo do YouTube" } : {}) },
    { id: "credential", title: "Credencial exclusiva", state: !integration || !credentials ? "blocked" : credentials.usable ? "configured" : "pending", detail: !integration ? blocked : !credentials ? "A emissão de credenciais não está disponível neste ambiente." : credentials.usable ? "Você tem uma credencial válida. Isso não confirma que o Streamer.bot esteja configurado." : "Crie uma credencial e salve o ID e o segredo no seu Streamer.bot.", href: `#integracao-${creator.id}`, link: "Gerenciar credenciais" },
