...
- Hosts da plataforma: origem canônica de `NEXT_PUBLIC_PLATFORM_URL` (por padrão, `ludylops-youtube-dashboard.vercel.app`), `localhost`, `127.0.0.1` e `::1`, além dos hosts exatos de `APP_URL`, `NEXT_PUBLIC_APP_URL`, `VERCEL_URL`, `VERCEL_BRANCH_URL` e `VERCEL_PROJECT_PRODUCTION_URL`. `ludylops.live` e `www.ludylops.live` são excluídos dessa classificação mesmo quando aparecem nessas variáveis. Não existe permissão genérica para `*.vercel.app`.
- Na plataforma, somente `/c/:slug` ou um slug explícito seleciona a comunidade; a raiz abre o Creator Hub. Em `ludylops.live` e `www.ludylops.live`, os links `/c/:slug` continuam válidos por compatibilidade. Somente nesses dois hosts Ludylops pode ser selecionada sem slug nos caminhos exatos `/`, `/apostas`, `/contadores`, `/indicacoes`, `/jogos`, `/me`, `/privacy`, `/produtinhos`, `/quotes`, `/ranking`, `/terms` e `/videos`.
